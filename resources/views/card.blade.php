...
<div class="row">
            ...
            <table class="table table-hover shopping-cart-wrap">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-right">
                            <a href="{{ route('checkout.cart.remove', $item->id) }}" class="btn btn-outline-danger"><i
    ...
    <aside class="col-sm-3">
        ...
        <dl class="dlist-align h4">
            <dt>Total:</dt>
            <dd class="text-right"><strong>{{ config('settings.currency_symbol') }}{{ \Cart::getSubTotal() }}</strong>
            </dd>
        </dl>
        <hr>
    ...
    $product = Shopper\Framework\Models\Shop\Product\Product::find($item->id); ?>
                            <tr>
                                <td>
                                    <figure class="itemside">
                                        <div class="aside"><img src="{{ $product?->getFirstImage()?->image_full_path }}"
                                                class="img-sm"></div>
                                        <figcaption class="info">
                                            <a href="{{route('product.show', $product->slug)}}"
                                                class="title text-dark">{{ Str::words($item->name,20) }}</a>
                                            @foreach($item->attributes as $key => $value)
                                            <dl class="dlist-inline small">
                                                <dt>{{ ucwords($key) }}: </dt>
                                                <dd>{{ ucwords($value) }}</dd>
                    ...
                    <div class="card-body border-top">
                        <form action="{{route('order.store')}}" method="post">
                            <button type="submit" class="btn btn-primary float-md-right"> Make Purchase <i
                                    class="fa fa-chevron-right"></i> </button>
                        </form>
                        <a href="{{route('home')}}" class="btn btn-light"> <i class="fa fa-chevron-left"></i> Continue
                            shopping </a>
                    </div>
                </div> <!-- card.// -->
                @endif
                ...
                <div class="card">
                    <div class="card-body">
                        <dl class="dlist-align">
                            <dt>Total:</dt>
                            <dd class="text-right  h5"><strong>{{ shopper_money_format(\Cart::getSubTotal())}}</strong>
                            </dd>
                        </dl>
                        <hr>
